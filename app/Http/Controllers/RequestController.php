...
class RequestController extends Controller
{
    ...
    public function get_request(Request $request)
    {
            ...
            case 'IDAManager':
                $data = $data->where(function ($q) {
                    $q->where("StatusId", RequestStatus::where("key", "New")->first()->id)
                        ...
                        ->Orwhere("StatusId", RequestStatus::where("key", "AcceptanceConfirmed")->first()->id)
                        ->Orwhere("StatusId", RequestStatus::where("key", "Returned")->first()->id);
                });
                break;
        ...
        if (!empty($sortColumn) || !empty($sortDirection)) {
            ...
            if(strtolower($sortDirection) == "asc"){
                $data=$data->sortBy($sortColumn)->values();
            }else{
                $data=$data->sortByDesc($sortColumn)->values();
            }
        }
        ...
        foreach ($data as $item) {
            ...
            if(!empty($cert)){
                foreach ($cert as $value) {
                    if(isset($value) && !empty($value['certificateNumber'])){
                        $certificate = CertificateType::where('id' , $value['certificateTypeId'])->first();
                        if($certificate->name == "Export Fund"){
                            $exportCert['id'] = $value->id;
                            $exportCert['certificateNumber'] = $value->certificateNumber;
                            $exportCert['certificateTypeId'] = $certificate->id;
                            $exportCert['certificateTypeNameAr'] = $certificate->nameAr;
                            $exportCert['certificateTypeNameEn'] = $certificate->name; 
                            $exportCert['expirationDate'] = CertificateService::calcExpiryDate($value);
 
                        }else{
                            $law5Cert['id'] = $value->id;
                            $law5Cert['certificateNumber'] = $value->certificateNumber;
                            $law5Cert['certificateTypeId'] = $certificate->id;
                            $law5Cert['certificateTypeNameAr'] = $certificate->nameAr;
                            $law5Cert['certificateTypeNameEn'] = $certificate->name;
                            $law5Cert['expirationDate'] = CertificateService::calcExpiryDate($value);
                        }
                    }
                }
            }
            array_push(
                $response,
                [
                    "law5Certificate" =>(!is_null($law5Cert))? $law5Cert : null,
                    "exportCertificate" =>(!is_null($exportCert))? $exportCert : null, 
                    "id" => $item['id'],
                    ...
                    "isRenewal" => ($item['isRenewal']) ? true : false,
      ...
      $user = userData();
        ...
        switch ($req->statusId) {
            case ($req->statusId == RequestStatus::where("key", "Accepted")->first()->id):
                ...
                $req->isIDAFeesPaid = 1; /////////////
                $req->isFEIFeesPaid = 1; ///////////
                $action->actionId  = Action::where("key", "Confirm")->first()->id;
    ...
    public function ChangeChamber(Request $request)
    {
        ...
        if ($newChamber->assessmentMethod == $currentChamber->assessmentMethod) {
            ...
            return response()->json(__("chamber.otherType"), 200);//changeChamberDone
        } else {
            return response()->json(__("chamber.otherType"), 400);
        }
    }
    ...
    public function getLatestReturnAction(Request $request, $requestId)
    {
        ...
        $actionIdForReturnedCase = Action::where("key", "Return")->first()->id;
        $item = RequestAction::with("actions") //, "user_by", "user_to")
    ...
    { 
        ...
        $sortDirection = (!empty($request->get("sortDirection"))) ? $request->get("sortDirection") : "asc";
        ...
        if (filled($sortDirection)) {
            $sortDirection = strtolower($sortDirection);
            if ($sortDirection !== "asc" && $sortDirection !== "desc") {

                return response()->json("Order direction must be asc or desc", Response::HTTP_BAD_REQUEST);
            }
        }
    ...
    public function CertificatedProducts_with_issue_certificate(Request $request)
    {
        ...
        foreach ($data->get() as $item) {
            if(!empty($item['certificate'])){
                $cert1 = $item['certificate'][0] ?? null; 
                $cert2 = $item['certificate'][1] ?? null; 
            }
            ...
            $obj=[
                ...
                "productName"  =>  $item['assessments']['productName'],                    
                "requestId"    =>  $item['id'],                    
                "exportFundCertificate" => $cert2? true:false,
                "law5Certificate" =>  $cert1?true:false                          
            ]; 
            if (!in_array($obj, $response)){
                array_push($response,$obj);
            }
