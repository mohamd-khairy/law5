...
<body class="container">
	<?php 
		...
		if (explode("/", $url)[2] == "localhost:8000") {
			$u=" ";
			if($manufactor){
				$m = "/storage/images/export.png";
			}else{
				$m = "/storage/images/export_man.png";
			}
		} else {
			$u = $app->make('url')->to('/public');
			if($manufactor == 1){
				$m = $app->make('url')->to('/public/storage/images/export_new.png');
			}else{
				$m = $app->make('url')->to('/public/storage/images/export_man_new.png');
			}
		}
	?>	
	<style>
	...
	<?php if($manufactor){ ?>
	<div style=" position: absolute;top:305px;right: 450px;"><?php if (!empty($assessment)) echo $assessment['manufactoringCompanyName']; ?> </div>
	<div style=" position: absolute;top:305px;right: 820px;"><?php if (!empty($assessment)) echo $assessment['manufactoringCompanyIndustrialRegistry']; ?> </div>
	<?php }?>
	...
	<div style=" font-size:10px;font-wight:bold;position: absolute;top:125px;right: 843px;"><?php if (!empty($certificate)) echo "EXP-$certificate->certificateNumber"; ?> </div>
